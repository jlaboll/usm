...
check "(7d) scp-form URL canonicalized to https identity" "$(cat "$TMP/sh_scp")" "https://github.com/you/repo"
( . "$USM_ROOT/lib/core/git.sh"
  printf '%s\n' "$(usm_url_normalize 'ssh://git@github.com/you/repo.git')" ) >"$TMP/sh_ssh"
check "(7d) ssh:// URL canonicalized to https identity" "$(cat "$TMP/sh_ssh")" "https://github.com/you/repo"
( . "$USM_ROOT/lib/core/git.sh"
...
( . "$USM_ROOT/lib/core/git.sh"
  a="$(usm_url_normalize 'git@github.com:you/repo.git')"
  b="$(usm_url_normalize 'https://github.com/you/repo')"
  [ "$a" = "$b" ] && printf 'same\n' || printf 'diff\n' ) >"$TMP/sh_same"
check "(7d) scp and https forms share one identity" "$(cat "$TMP/sh_same")" "same"
( . "$USM_ROOT/lib/core/git.sh"
